...
imgNumber = 0
hs, ws = int(70 * 3.5), int(126 * 2.5)
#hs, ws = int(70 * 11.5), int(126 * 12.5)#
...
closeGesture = [1, 1, 1, 1, 1] 
def close_png_file():
    cv2.destroyAllWindows()
    print("PNG file closed.")
while True:
    ...
    if hands and buttonPressed is False:
        ...
        #gesture 4-close
        if fingers == closeGesture:
            print('Closing  file')
            pyautogui.press('q')
            buttonPressed = True

        #Gesture 5 - Drawing on the Slide
    ...
    imgCurrent = cv2.resize(imgCurrent, (int(width * 1.0), int(height * 1.0)))
